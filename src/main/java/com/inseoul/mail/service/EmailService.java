...
public class EmailService {
    ...
    @Autowired
    public EmailService(SqlSession sqlSession){
    ...
    private String tempPass; // 랜덤 임시 비번
    ...
    /**
     * 임시로 생성된 비밀번호를 encoding해서 유저의 비밀번호를 변경하는 것, 추후 유저 정보수정도 이렇게 해야함
     */
    public MimeMessage getUserPass(String email) throws MessagingException, UnsupportedEncodingException {

        createTempPassword();
        User user = userRepository.findByUsername(email);
        System.out.println(tempPass);
        user.setPassword(passwordEncoder.encode(tempPass));
        userRepository.tempPass(user);
    ...
    /**
     * 이메일 인증코드 받는거랑 똑같음, 단지 이건 임시비밀번호를 보내는 용도의 차이만 있음
     * @param toEmail
     * @return
     * @throws MessagingException
     * @throws UnsupportedEncodingException
     */
    public AuthCodeQryResult sendPass(String toEmail) throws MessagingException, UnsupportedEncodingException {

        //메일전송에 필요한 정보 설정
        MimeMessage emailForm = getUserPass(toEmail);
        //실제 메일 전송 참고로 send()메소드에 담긴 매개변수는 createMimeMessage()로 만들어진 MimeMessage타입이여만 함 (당연히 둘은 같은 클래스에있기 때문일듯?)
        emailSender.send(emailForm);
        AuthCodeQryResult authCodeQryResult = AuthCodeQryResult.builder()
                .status("OK")
